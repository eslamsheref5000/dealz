        ...
        <div className="flex overflow-x-auto pb-4 gap-4 px-2 no-scrollbar scroll-smooth snap-x">
            {categories.map((cat) => (
                <button
                    ...
                    className={`snap-center flex flex-col items-center gap-2 min-w-[80px] group transition-all duration-500 p-3 rounded-2xl border-2 hover:translate-y-[-4px] ${selectedCategory === cat
                        ? "border-red-600 bg-red-50/50 dark:bg-red-900/10 scale-110 shadow-lg shadow-red-100 dark:shadow-none"
                        : "border-transparent hover:bg-white dark:hover:bg-gray-800 hover:shadow-xl hover:shadow-gray-200/50 dark:hover:shadow-none"
                        }`}
                >
                    <div
                    ...
                    <span
                        className={`text-xs font-black tracking-tight text-center whitespace-nowrap px-1 transition-colors duration-300 ${selectedCategory === cat ? "text-red-600" : "text-gray-500 dark:text-gray-400 group-hover:text-red-500"
                            }`}
                    >
                        {t(`categories.${cat}`)}
                    </span>
                    {selectedCategory === cat && (
                        <div className="w-1.5 h-1.5 rounded-full bg-red-600 animate-pulse mt-0.5"></div>
                    )}
                </button>
